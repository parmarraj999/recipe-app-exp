import { View, Text, ImageBackground, Image, StyleSheet, ScrollView, Pressable } from 'react-native'
...
const DetailPage = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`https://api.spoonacular.com/recipes/${id}/information?apiKey=7cb3d33c08a64d6e9ed4b53ce1041581`);
          if (!response.ok) {
                ...
                data ? (
                    <View style={style.container} >
                        <View style={style.header} >
                            <BackButton />
                            <Text style={{ position: 'absolute', left: '50%', fontSize: 18, fontWeight: 600, transform: 'translate(-50%,0)' }}>Recipe</Text>
                        </View>
                        ...
                        <View style={style.recipeDetail} >
                            <View style={{ alignItems: 'center' }}>
                                ...
                                <Text style={style.time}>{data.readyInMinutes} Min</Text>
                            </View>
                            <View style={{ alignItems: 'center' }} >
                                <Text style={style.prepText}>Preparation</Text>
                                <Text style={style.time}>{data.preparationMinutes} Min</Text>
                            </View>
                            <View style={{ alignItems: 'center' }} >
                                <Text style={style.prepText}>Cooking</Text>
                                <Text style={style.time}>{data.cookingMinutes} Min</Text>
                            </View>
                        </View>
                        <View style={style.ingContainer} >
                            <View style={style.ingHeader} >
                                <Text style={{ fontSize: 22, fontWeight: 600 }} >Ingredients</Text>
                                <Text style={{ fontSize: 14, color: 'grey' }}>{data.extendedIngredients.length} Healthy ingredients</Text>
                            </View>
                            <View style={style.ingList} >
                                {
                                    data?.extendedIngredients.map((data: any, index: number) => (
                                        <View key={index} style={style.ingItem}>
                                            <View>
                                                <Text style={{fontSize:16,fontWeight:600}}>{data.name}</Text>
                                            </View>
                                            <Text>{data.measures.metric.amount} {data.measures.metric.unitShort}</Text>
                                        </View>
                                    ))
                                }
                            </View>
